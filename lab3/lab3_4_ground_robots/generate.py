...
parser.add_option('-b', '--groundrobots', metavar='NUM', type=int, dest='groundrobots', help='the number of ground robots')

(options,args) = parser.parse_args()

if (options.helicopters == None or options.carriers == None or
    options.locations == None or options.persons == None or
    options.crates == None or options.goals == None or
    options.groundrobots == None):
...
for x in range(options.groundrobots):
...
problem_name ="helicopter_problem_"+\
str(options.helicopters)+"_"+\
str(options.groundrobots)+"_"+\
str(options.carriers)+"_"+\
str(options.crates)+"_"+\
str(options.goals)
#"c"+("c".join([str(x) for x in num_crates_with_contents]))+
#"_"+str(options.locations)+"_"+str(options.persons)+
...
for (x, lx) in enumerate(location):
	for (y, ly) in enumerate(location):
		f.write("\t(= (drive-cost " + lx + " " + ly + ") " + str(int(random.uniform(0.9, 1.3) * flight_cost(x, y))) + ")\n")
